...
export const updateError = <T, E, D, E2>(state: ParserState<T, E, D>, error: E2): ParserState<T, E2, D> => ({
	...state,
	isError: true,
	error,
});
...
export const updateResult = <T, E, D, T2>(state: ParserState<T, E, D>, result: T2): ParserState<T2, E, D> => ({
	...state,
	result,
});

// updateData :: (ParserState e a s, t) -> ParserState e b t
export const updateData = <T, E, D, D2>(state: ParserState<T, E, D>, data: D2): ParserState<T, E, D2> => ({
	...state,
	data,
});
...
export class Parser<T, E = string, D = any> {
	...
	errorChain<T2, E2>(fn: (error: Err<E, D>) => Parser<T2, E2, D>): Parser<T2, E2, D> {
		const p = this.p;
		return new Parser(function Parser$errorChain$state(state): ParserState<T2, E2, D> {
			const nextState = p(state);
	...
	chainFromData<T2, E2>(fn: (data: { result: T; data: D }) => Parser<T2, E2, D>): Parser<T2, E2, D> {
		const p = this.p;
		return new Parser(function Parser$chainFromData$state(state): ParserState<T2, E2, D> {
			const newState = p(state);
